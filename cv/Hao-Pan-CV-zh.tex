...
\section{个人简介}
有技术教学、数据分析和人工智能应用开发经历，习惯先把复杂问题拆清楚，再做成能够验证的流程。做项目时，我会先确认真正的问题、使用场景和判断标准，再决定哪些环节适合交给人工智能；功能做出来只是起点，更重要的是它能不能被使用、结果能不能复核。能够与产品经理和业务方对齐需求，也能完成从原型、前后端开发到评估迭代的落地，并用明确规则、失败检查和人工确认保证关键环节可靠。
...
\begin{roleItems}
  \item 主导搭建 Analysis KB 内部 Web 应用，使用 Svelte 和 Python 实现前后端，通过 GitLab 读取历史 Case，并接入 AI 完成相似 Case 检索、人工选择和 Request Draft 生成。
